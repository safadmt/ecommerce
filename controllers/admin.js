...
export const getHomePage = async (req, res, next) => {
  ...
  try {
    ...
    const totalsales = await totalOrders({
...
export const createNewProduct = (req, res, next) => {
  ...
  const { discount_in_percentage } = req.body;
  req.body.discount_in_percentage = discount_in_percentage
  ...
  insertProduct(req.body) // Insert product into the database
    .then((data) => {
      if (data) {
        return res.status(200).json("Ok"); // Respond with "Ok" if insertion is successful
      }
    })
    .catch((err) => {
...
export const productIsAcitve = async (req, res, next) => {
  ...
  if (isValidObjectId(req.params.productid)) {
    ...
    try {
      const response = await productAcitveorInactive(
        // Update the active status based on request body
...
export function getAddCouponPage(req, res) {
  ...
  }); // Render the add coupon page with the role
}
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          message:
            "Cannot change the order status due to the order is already shipped.",
        });
    }
...
export async function removeUser(req, res) {
  try {
    ...
    if (response.error) {
      ...
      return res
        .status(200)
        .json({ error: null, data: "Ok", newusers, totalusers });
    }
  } catch (err) {
    return res.status(500).json("Something went wrong. Please try again later");
  }
...
export async function getReturnsListingPage(req, res, next) {
  ...
  try {
    ...
    const totalreturns = await countProductReturnDocuments({
      returnStatus: returnStatusEnum.COMPLETED,
    });
    if (returns) {
...
export async function handleReturnStatus(req, res, next) {
  ...
  try {
    ...
    const currentStatusIndex = returnStatus.indexOf(
      productreturned.returnStatus
    );
    ...
    if (newStatusIndex == -1) {
      return res
        .status(400)
        .json({ message: "Invalid status " + return_status });
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json({
          message: `Invalid status changes from ${productreturned.returnStatus} to ${return_status}`,
        });
    }
    ...
    if (return_status === returnStatusEnum.REFUND_PROCESSED) {
      ...
      const returedproduct = order.products.find(
        (item) =>
          item.productid._id.toString() === productreturned.productid.toString()
      );
      ...
      await updateOrderProductReturn(
        order._id,
        product._id,
        productreturned.quantity
      );
      await product.save();
      // Update the order with the new total price
      if(order.products.length === 1) {
        await updateOrder(order._id, { totalPrice: 0 });
      }else {
